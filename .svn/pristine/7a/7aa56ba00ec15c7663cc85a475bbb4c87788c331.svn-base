 ...
 * @date 2016年5月16日
 */
@Service("minicarCustomerService")
public class MiniCarCustomerServcieImpl {
	...
	//	tradeDto.setTradeStaffId("1001");
	    ...
	    tfBTradePoMapper.insert(tradePo);
		...
		tfBTradeCustomerPoMapper.insert(custPo);
		 //台账账户po
	    ...
	    tfBTradeAcctPoMapper.insert(acctPo);
		//台账集团po
	   // dto.setBusiLicenceValidDate(date); //页面问题时间无法传 手动模拟添加 后期改掉
	    ...
	    tfBTradeGroupPoMapper.insert(groupPo);
